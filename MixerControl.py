...
class MixerControl(Control):
    ...
    def get_midi_bindings(self):
        return (
            ...
            (
                "toggle_crossfade_a",
                lambda value, mode, status: self.toggle_crossfade(
                    MixerDevice.MixerDevice.crossfade_assignments.A
                ),
            ),
            (
                "toggle_crossfade_b",
                lambda value, mode, status: self.toggle_crossfade(
                    MixerDevice.MixerDevice.crossfade_assignments.B
                ),
            ),
            (
                "assign_crossfade_none",
                lambda value, mode, status: self.toggle_crossfade(
                    MixerDevice.MixerDevice.crossfade_assignments.NONE
                ),
            ),
    ...
    def toggle_arm_exclusive(self, value, mode, status):
        if status == MIDI.CC_STATUS and not value:
            return
        # toggle exclusive depending on settings
        self.toggle_arm_track(
            self.song.view.selected_track, (not self.song.exclusive_arm)
        )
    ...
    def toggle_solo_exclusive(self, value, mode, status):
        if status == MIDI.CC_STATUS and not value:
            return
        # toggle exclusive depending on settings
        self.toggle_solo_track(
            self.song.view.selected_track, (not self.song.exclusive_solo)
        )
    ...
    def reset_volume(self, value, mode, status):
        self.song.view.selected_track.mixer_device.volume.value = (
            settings.volume_default
        )
    ...
    def switch_monitoring(self, value, mode, status):
        ...
        if hasattr(track, "current_monitoring_state"):
            track.current_monitoring_state = (track.current_monitoring_state + 1) % len(
                track.monitoring_states.values
            )
    ...
    def input_rotate(self, value, mode, status):
        track = self.song.view.selected_track
        track.current_input_routing = track.input_routings[
            self.get_routing_index(
                value, mode, status, track.current_input_routing, track.input_routings
            )
        ]

    def input_sub_rotate(self, value, mode, status):
        track = self.song.view.selected_track
        track.current_input_sub_routing = track.input_sub_routings[
            self.get_routing_index(
                value,
                mode,
                status,
                track.current_input_sub_routing,
                track.input_sub_routings,
            )
        ]
    ...
    def output_rotate(self, value, mode, status):
        track = self.song.view.selected_track
        track.current_output_routing = track.output_routings[
            self.get_routing_index(
                value, mode, status, track.current_output_routing, track.output_routings
            )
        ]

    def output_sub_rotate(self, value, mode, status):
        track = self.song.view.selected_track
        track.current_output_sub_routing = track.output_sub_routings[
            self.get_routing_index(
                value,
                mode,
                status,
                track.current_output_sub_routing,
                track.output_sub_routings,
            )
        ]
