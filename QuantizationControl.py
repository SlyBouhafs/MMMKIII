...
class QuantizationControl(Control):
    ...
    def __init__(self, c_instance, selected_track_controller):
        ...
        self._midi_recording_quantization_labels = [
            "None",
            "1/4",
            "1/8",
            "1/8T",
        ...
        for key, max_steps in (
            ("clip_trigger_quantizations", 13),
            ("midi_recording_quantizations", 8),
        ):
    ...
    def clip_trigger_quantization(self, value, mode, status):
        ...
        self.song.clip_trigger_quantization = self._get_quantization(
            value,
            mode,
            status,
            self.song.clip_trigger_quantization,
            settings.clip_trigger_quantization_steps,
        )
    ...
    def midi_recording_quantization(self, value, mode, status):
        ...
        self.song.midi_recording_quantization = self._get_quantization(
            value,
            mode,
            status,
            self.song.midi_recording_quantization,
            settings.midi_recording_quantization_steps,
        )
    ...
    def _get_quantization(
        self, value, mode, status, current_quantization, quantizations
    ):
